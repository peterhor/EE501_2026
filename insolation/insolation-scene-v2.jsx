...
// ── Merged heliocentric + axial-parallelism figure ───────────────────────────
// One oblique view of the orbit (after the reference stills): Sun fixed at the focus,
// Earth at its true Kepler position, and the four season stations each drawn with the
// SAME fixed-direction axis. Sphere shading is a real 3D lighting calculation, so the
// far station reads mostly-lit and the near one mostly-dark instead of the meaningless
// top/bottom split a flat projection produces.
const SPACE_BG = '#0b1226';
const SPACE_GRID = '#5d6b96';
const SPACE_TEXT = '#c8d2ea';
const SPACE_DIM = '#8b97ba';
const SUN_CORE = '#fff3c4';
const LIT = '#bcdcef';
const NIGHT_SIDE = '#16223c';
...
function MergedOrbitPanel({ nday, deltaDeg, exaggerateOrbit }) {
  ...
  const rfOf = (A) => rOfV((A - PERI_A) * D2R, eccUsed);
  const posOf = (A, radius) => {
    const rf = rfOf(A);
    return [ocx + rx * rf * Math.cos(A * D2R), ocy - (radius !== undefined ? radius : ryBase) * rf * Math.sin(A * D2R)];
  };

  // lighting for a station at screen angle A — depends only on A
  const lightOf = (A) => {
    const a = A * D2R;
    return {
      lx: -Math.cos(a),                     // screen x component of Earth→Sun
      ly: Math.sin(a) * sinE,               // screen y (down positive)
      lz: Math.sin(a) * cosE,               // toward the viewer
    };
  };
  ...
  for (let A = 0; A <= 360; A += 3) {
    const [x, y] = posOf(A);
    orbitPts.push(`${x.toFixed(1)} ${y.toFixed(1)}`);
  }
  const orbitPath = 'M ' + orbitPts.join(' L ') + ' Z';

  // live Earth
  const v = trueAnomaly(nday);
  const liveA = (trueLongitudeDeg(v) + 90) % 360;
  const [ex, ey] = posOf(liveA);
  const liveLight = lightOf(liveA);

  const stations = [
    { A: 180, label: 'June solstice', sub: 'N pole leans sunward' },
    { A: 270, label: 'September equinox', sub: null },
    { A: 0, label: 'December solstice', sub: 'N pole leans away' },
    { A: 90, label: 'March equinox', sub: null },
  ].map((s) => {
    const [x, y] = posOf(s.A);
    const dist = Math.hypot(x - ex, y - ey);
    // yield to the live Earth: the stations sit ON the orbit it travels
    return { ...s, x, y, light: lightOf(s.A), dist, fade: clamp((dist - 36) / 40, 0, 1) };
  });

  const axisPts = (x, y, len) => {
    const dx = Math.sin(AXIS_TILT * D2R), dy = -Math.cos(AXIS_TILT * D2R);
    return { n: [x + dx * len, y + dy * len], s: [x - dx * len, y - dy * len], dx, dy };
  };
  const arrowHead = (x, y, len) => {
    const { n, dx, dy } = axisPts(x, y, len);
    const bx = n[0] - dx * 9, by = n[1] - dy * 9;
  ...
  const EarthGlyph = ({ id, x, y, r, light, showEquator, minLit, opacity }) => {
    ...
    return (
      <g opacity={opacity === undefined ? 1 : opacity}>
        ...
        <line x1={ax.s[0]} y1={ax.s[1]} x2={ax.n[0]} y2={ax.n[1]} stroke="#f2f5fb" strokeWidth={2} />
        <path d={arrowHead(x, y, r + 15)} fill="#f2f5fb" />
      </g>
    );
  };
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" height="100%">
      <rect x="0" y="0" width={W} height={H} rx="6" fill={SPACE_BG} />

      <defs>
        <radialGradient id="sunGlow">
          <stop offset="0%" stopColor={SUN_CORE} stopOpacity="1" />
          <stop offset="42%" stopColor="#ffe9a3" stopOpacity="0.85" />
          <stop offset="100%" stopColor="#ffd45e" stopOpacity="0" />
        </radialGradient>
      </defs>

      <text x={16} y={24} fontFamily={SANS} fontSize={14} fill={SPACE_TEXT}>
        Earth orbits the fixed Sun — and its axis always points the same way
      </text>
      <text x={16} y={42} fontFamily={SANS} fontSize={11.5} fill={SPACE_DIM}>
        eqs. 2.14–2.19 · ▲ = North-pole end of the rotation axis · dashed line = equator
      </text>

      <path d={orbitPath} fill="none" stroke={SPACE_GRID} strokeWidth={1.2} strokeDasharray="5 5" opacity={0.85} />

      <circle cx={ocx} cy={ocy} r={86} fill="url(#sunGlow)" />
      <circle cx={ocx} cy={ocy} r={28} fill={SUN_CORE} />
      <text x={ocx} y={ocy + 52} textAnchor="middle" fontFamily={SANS} fontSize={12} fontWeight={700} fill="#ffe9a3">SUN</text>

      {/* season stations — same arrow, four places. A ring marker always holds the spot;
          the globe itself fades as the live Earth drives through it. */}
      {stations.map((s, i) => {
        const above = s.A === 90;
        const ly = above ? s.y - 27 - 26 : s.y + 27 + 24;
        const lx = clamp(s.x, 58, W - 58);
        return (
          <g key={i}>
      ...
      <text x={16} y={H - 16} fontFamily={SANS} fontSize={11.5} fill={SPACE_DIM}>
        Sub-solar latitude δ = {deltaDeg.toFixed(1)}° · r/a = {rOfV(v, ECC).toFixed(4)} · the Sun never moves; only Earth&rsquo;s place in the orbit does
      </text>
    </svg>
  );
}
...
function DiagramPanel({ phiDeg, deltaDeg }) {
  ...
  return (
    <svg viewBox="0 0 740 600" width="100%" height="100%">
      ...
      {/* Earth circle */}
      <circle cx={cx} cy={cy} r={R} fill="#f7fbfe" stroke={INK} strokeWidth={2} />
      <clipPath id="earthClip"><circle cx={cx} cy={cy} r={R} /></clipPath>
      {/* faint rotating-Earth backdrop: turns with the tilted globe. The source gif already
          spins prograde (west-to-east) for a fixed external viewer with north up — do NOT
          mirror this group, a horizontal flip reverses the apparent spin to retrograde. */}
      <g clipPath="url(#earthClip)" opacity={0.17}>
        <g transform={`rotate(${deltaDeg} ${cx} ${cy})`}>
          <image href={(typeof window !== 'undefined' && window.EARTH_ROTATION_GIF) || 'uploads/earth_rotation.gif'} x={-35.7} y={96.2} width={523.5} height={430.3} preserveAspectRatio="none" />
        </g>
      </g>
