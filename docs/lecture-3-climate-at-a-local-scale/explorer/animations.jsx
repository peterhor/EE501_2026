...
function Stage({
  ...
  // Ctrl/Cmd + wheel and Ctrl/Cmd +/-/0 zoom the canvas (browser zoom alone cannot,
  // because the auto-fit above rescales to whatever viewport size the browser reports).
  React.useEffect(() => {
    const el = stageRef.current;
    if (!el) return;
    const onWheel = (e) => {
      ...
      e.preventDefault();
      setZoom((z) => clamp(z * (e.deltaY < 0 ? 1.12 : 1 / 1.12), 0.25, 6));
    };
    const onKey = (e) => {
      if (!(e.ctrlKey || e.metaKey)) return;
      if (e.key === '+' || e.key === '=') { e.preventDefault(); setZoom((z) => clamp(z * 1.15, 0.25, 6)); }
      else if (e.key === '-' || e.key === '_') { e.preventDefault(); setZoom((z) => clamp(z / 1.15, 0.25, 6)); }
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        width: '100%',
        position: 'relative',
        display: 'flex', alignItems: zoom > 1 ? 'flex-start' : 'center',
        ...
        {/* Zoom control — Ctrl/⌘ + wheel does the same thing, but the pill makes it discoverable
            and gives a readout, which matters for a figure meant to be inspected in a lecture. */}
        <div style={{
          position: 'absolute', top: 12, right: 16, zIndex: 5,
          display: 'flex', alignItems: 'center', gap: 2,
          background: 'rgba(18,18,18,0.82)', border: '1px solid #3a3a3a',
          borderRadius: 999, padding: '3px 4px',
          fontFamily: 'Inter, system-ui, sans-serif', color: '#e8e6df',
        }}>
          <button title="Zoom out (Ctrl/⌘ −)" onClick={() => setZoom((z) => clamp(z / 1.15, 0.25, 6))} style={zoomBtnStyle}>−</button>
          <button title="Reset zoom (Ctrl/⌘ 0)" onClick={() => setZoom(1)} style={{ ...zoomBtnStyle, width: 56 }}>{Math.round(zoom * 100)}%</button>
          <button title="Zoom in (Ctrl/⌘ +)" onClick={() => setZoom((z) => clamp(z * 1.15, 0.25, 6))} style={zoomBtnStyle}>+</button>
        </div>
      </div>
...
const zoomBtnStyle = {
  minWidth: 26, height: 24, padding: '0 8px',
  background: 'transparent', border: 'none', borderRadius: 999,
  color: '#e8e6df', fontSize: 13, fontWeight: 600, lineHeight: 1, cursor: 'pointer',
